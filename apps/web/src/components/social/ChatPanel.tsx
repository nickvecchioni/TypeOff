    ...
    <div className="flex flex-col h-full">
      ...
      >
        ...
        {loadingHistory && messages.length === 0 && (
          <p className="text-xs text-muted/30 text-center py-8">Loading...</p>
        )}
        ...
        {!loadingHistory && messages.length === 0 && (
          <div className="flex flex-col items-center justify-center py-12 gap-2">
            ...
            <p className="text-xs text-muted/25">No messages yet</p>
          </div>
        )}
              ...
              >
                ...
                <p
                  className={`text-[10px] mt-0.5 tabular-nums ${
                    isMine ? "text-accent/30 text-right" : "text-muted/30"
                  }`}
                >
      ...
      <div className="px-3 py-2.5 border-t border-white/[0.06]">
        <div className="flex gap-2">
          <input
            ...
            placeholder="Message..."
            maxLength={500}
            className="flex-1 bg-surface/60 text-text text-sm rounded-md px-3 py-2 outline-none ring-1 ring-white/[0.06] focus:ring-accent/25 transition-all placeholder:text-muted/30"
          />
          <button
            onClick={handleSend}
            disabled={!input.trim()}
