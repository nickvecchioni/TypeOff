  ...
  return (
    <div className="relative rounded-lg overflow-hidden glow-gm">
      ...
      <div className="bg-rank-grandmaster/[0.06] px-5 py-5">
        ...
        <p className="text-sm text-muted/70 mt-2">{tier.flavor}</p>
      </div>
    </div>
  );
}
      ...
      <div className="pl-5 pr-4 py-3.5">
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="text-right">
            ...
            <span className="block text-xs text-muted/60 tabular-nums">
              {tier.wpm} wpm
            </span>
          </div>
        </div>
        <p className="text-sm text-muted/65 mt-1.5">{tier.flavor}</p>
      </div>
    </div>
  );
}
...
function InfoCard({
  ...
  return (
    <div className="rounded-lg bg-surface/25 ring-1 ring-white/[0.04] px-4 py-3.5">
      <h3 className="text-xs font-bold text-muted/65 uppercase tracking-widest mb-2">
        {title}
      </h3>
      <p className="text-sm text-text/60 leading-relaxed">{children}</p>
    </div>
  );
}
