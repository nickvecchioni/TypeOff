...
async function RankedLeaderboard({ userId, db, universe }: { userId?: string; db: ReturnType<typeof getDb>; universe?: string }) {
  // When a specific mode is selected, show per-mode ELO rankings
  if (universe && universe !== "all") {
    return <ModeEloLeaderboard userId={userId} db={db} modeCategory={universe} />;
  }

  // "All" view: show best (display) ELO across all modes
  const rows = await db
    .select({
      id: users.id,
    ...
    .where(and(isNotNull(users.username), gt(users.eloRating, 1000)))
